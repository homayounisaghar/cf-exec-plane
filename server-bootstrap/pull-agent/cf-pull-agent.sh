...
install -d -m 0750 -o root -g root "$RELEASES" "$STATE" /var/log/capability-fabric /var/lib/capability-fabric/agent-home
...
read_state() {
  ...
  if [[ -r "$path" ]]; then cat "$path"; else printf '%s' "$fallback"; fi
}
manifest_value() {
...
compose_apply() {
  ...
  docker compose -p "$PROJECT" -f "$release/compose.yaml" up -d --remove-orphans --no-build >>"$DETAIL_LOG" 2>&1
}
compose_pull() {
  local release="$1"
  detail "compose pull"
...
rollback_to() {
  ...
  if [[ -n "$old" && -d "$old" && -s "$old/compose.yaml" && -s "$old/health.sh" ]]; then
    ...
    if compose_apply "$old" && health_check "$old"; then
      detail "rollback success"
      echo "CF_PULL_ROLLBACK=success"
      return 0
    fi
    detail "rollback previous health/apply failed"
    echo "CF_PULL_ROLLBACK=failed" >&2
    return 1
  fi
  rm -f "$ACTIVE"
  ...
  echo "CF_PULL_ROLLBACK=unavailable" >&2
  return 1
}
...
if [[ "$MODE" == rollback-drill ]]; then
  ...
  saved_previous=''
  [[ -L "$PREVIOUS" ]] && saved_previous="$(readlink -f "$PREVIOUS")"
...
DRILL
  ...
  atomic_link "$drill" "$ACTIVE"
  if ! compose_apply "$drill"; then
    detail "rollback drill candidate apply failed before forced health"
  fi
  if health_check "$drill"; then
    atomic_link "$old" "$ACTIVE"
    compose_apply "$old" || true
    rm -rf "$drill"
    echo "CF_ROLLBACK_DRILL_UNEXPECTED_HEALTH_PASS" >&2
    exit 61
  fi
  if ! rollback_to "$old" "$drill"; then
    rm -rf "$drill"
    echo "CF_ROLLBACK_DRILL=failed" >&2
    exit 62
  fi
  if [[ -n "$saved_previous" && -d "$saved_previous" ]]; then
    atomic_link "$saved_previous" "$PREVIOUS"
  else
    rm -f "$PREVIOUS"
  fi
  rm -rf "$drill"
  echo "CF_ROLLBACK_DRILL=success"
  exit 0
fi

# Git asks this helper for credentials. The token itself never appears in a
# command argument, URL, repository config, or log.
...
if [[ ! -d "$CACHE" ]]; then
  git init --bare "$CACHE" >>"$DETAIL_LOG" 2>&1
fi
if git --git-dir="$CACHE" remote get-url origin >/dev/null 2>&1; then
  git --git-dir="$CACHE" remote set-url origin "$REPO_URL"
else
  git --git-dir="$CACHE" remote add origin "$REPO_URL"
fi

detail "fetch start"
if ! GIT_ASKPASS="$askpass" GIT_TERMINAL_PROMPT=0 HOME=/var/lib/capability-fabric/agent-home \
  git --git-dir="$CACHE" fetch --quiet --force --prune --depth=1 origin \
  "refs/heads/${BRANCH}:refs/remotes/origin/${BRANCH}" >>"$DETAIL_LOG" 2>&1; then
  echo "CF_PULL_FETCH_FAILED" >&2
  exit 30
fi
...
if [[ "$commit" == "$last_good_commit" ]]; then
  echo "CF_PULL_NO_CHANGE"
  exit 0
fi
if [[ "$commit" == "$last_failed_commit" ]]; then
  echo "CF_PULL_SKIPPED_PREVIOUSLY_FAILED_HEAD"
  exit 0
fi

for f in manifest.json manifest.json.sig; do
  if ! git --git-dir="$CACHE" show "$commit:$DEPLOY_DIR/$f" > "$work/$f" 2>>"$DETAIL_LOG"; then
    atomic_write "$STATE/last-failed-commit" "$commit"
    echo "CF_PULL_NO_SIGNED_CANDIDATE" >&2
    exit 31
  fi
done
...
if [[ "$key_type" != ssh-ed25519 || -z "$key_data" || -n "${extra:-}" ]]; then
  echo "CF_PULL_INVALID_TRUST_KEY" >&2
  exit 21
fi
...
if ! ssh-keygen -Y verify -f "$allowed" -I "$SIGN_ID" -n "$SIGN_NAMESPACE" -s "$work/manifest.json.sig" < "$work/manifest.json" >>"$DETAIL_LOG" 2>&1; then
  atomic_write "$STATE/last-failed-commit" "$commit"
  echo "CF_PULL_SIGNATURE_REJECTED" >&2
  exit 32
fi
...
if set(m) != expected:
    raise SystemExit('manifest keys mismatch')
if m['schema']!='capability-fabric.deploy.v1':
    raise SystemExit('schema mismatch')
if not isinstance(m['sequence'],int) or not (1 <= m['sequence'] <= 9223372036854775807):
    raise SystemExit('bad sequence')
if not isinstance(m['release_id'],str) or not re.fullmatch(r'[A-Za-z0-9][A-Za-z0-9._-]{0,79}',m['release_id']):
    raise SystemExit('bad release_id')
if m['compose_file']!='compose.yaml' or m['health_file']!='health.sh':
    raise SystemExit('fixed file names required')
if not isinstance(m['health_timeout_seconds'],int) or not (5 <= m['health_timeout_seconds'] <= 300):
    raise SystemExit('bad health timeout')
files=m['files']
if not isinstance(files,dict) or not files:
    raise SystemExit('bad files')
for required in ('compose.yaml','health.sh'):
    if required not in files:
        raise SystemExit('missing required file hash')
for p,h in files.items():
    if not isinstance(p,str) or p.startswith('/') or '..' in p.split('/') or not re.fullmatch(r'[A-Za-z0-9._/-]+',p):
        raise SystemExit('unsafe file path')
    if not isinstance(h,str) or not re.fullmatch(r'[0-9a-f]{64}',h):
        raise SystemExit('bad file hash')
images=m['images']
if not isinstance(images,list) or not images or len(set(images))!=len(images):
    raise SystemExit('bad images')
for image in images:
    if not isinstance(image,str) or not re.fullmatch(r'[^\s]+@sha256:[0-9a-f]{64}',image):
        raise SystemExit('image not digest pinned')
with open(meta_path,'w',encoding='utf-8') as f:
    f.write(f"{m['sequence']}\n{m['release_id']}\n{m['health_timeout_seconds']}\n")
...
then
  atomic_write "$STATE/last-failed-commit" "$commit"
  echo "CF_PULL_MANIFEST_REJECTED" >&2
  exit 33
fi
...
if (( sequence < last_sequence )); then
  atomic_write "$STATE/last-failed-commit" "$commit"
  echo "CF_PULL_REPLAY_REJECTED" >&2
  exit 35
fi
if (( sequence == last_sequence )); then
  last_release="$(read_state "$STATE/last-good-release" '')"
  if [[ "$release_id" == "$last_release" ]]; then
    atomic_write "$STATE/last-good-commit" "$commit"
    echo "CF_PULL_NO_CHANGE"
    exit 0
  fi
  atomic_write "$STATE/last-failed-commit" "$commit"
  echo "CF_PULL_SEQUENCE_COLLISION" >&2
  exit 35
fi
...
while IFS=$'\t' read -r rel expected_hash; do
  ...
  if ! git --git-dir="$CACHE" show "$commit:$DEPLOY_DIR/$rel" > "$stage/$rel" 2>>"$DETAIL_LOG"; then
    rm -rf "$stage"
    atomic_write "$STATE/last-failed-commit" "$commit"
    echo "CF_PULL_RELEASE_FILE_MISSING" >&2
    exit 37
  fi
  ...
  if [[ "$actual_hash" != "$expected_hash" ]]; then
    rm -rf "$stage"
    atomic_write "$STATE/last-failed-commit" "$commit"
    echo "CF_PULL_RELEASE_HASH_MISMATCH" >&2
    exit 38
  fi
...
for name,svc in services.items():
    if 'build' in svc:
        raise SystemExit(f'build forbidden: {name}')
    image=svc.get('image')
    if not isinstance(image,str) or not image:
        raise SystemExit(f'image required: {name}')
    actual.append(image)
with open(sys.argv[2],encoding='utf-8') as f:
    expected=[x.strip() for x in f if x.strip()]
if sorted(set(actual)) != sorted(expected):
    raise SystemExit('resolved image set differs from signed manifest')
PY
then
  rm -rf "$stage"
  atomic_write "$STATE/last-failed-commit" "$commit"
  echo "CF_PULL_IMAGE_POLICY_REJECTED" >&2
  exit 39
fi
...
if ! compose_pull "$release"; then
  atomic_write "$STATE/last-failed-commit" "$commit"
  echo "CF_PULL_IMAGE_PULL_FAILED" >&2
  exit 40
fi
...
if [[ -L "$ACTIVE" ]]; then
  old="$(readlink -f "$ACTIVE")"
  [[ "$old" == "$RELEASES/"* && -d "$old" ]] || { atomic_write "$STATE/last-failed-commit" "$commit"; echo "CF_PULL_INVALID_CURRENT_POINTER" >&2; exit 41; }
fi
...
if ! compose_apply "$release" || ! health_check "$release"; then
  ...
  if rollback_to "$old" "$release"; then
    echo "CF_PULL_APPLY_FAILED_ROLLED_BACK" >&2
    exit 42
  fi
  echo "CF_PULL_APPLY_FAILED_ROLLBACK_FAILED" >&2
  exit 43
fi
...
detail "activation success sequence=$sequence"
echo "CF_PULL_APPLY=success"

...
python3 - "$source_manifest" / "${roots[@]}" <<'PY'
import hashlib,os,stat,sys
out,base,*roots=sys.argv[1:]
def digest_file(p):
    h=hashlib.sha256()
    with open(p,'rb') as f:
...
PY
restic backup --tag capability-fabric-personal-server "${roots[@]}" >>"$log" 2>&1
...
if [[ "$mode" == prove ]]; then
  ...
  python3 - "$restored_manifest" "$restore_dir" "${roots[@]}" <<'PY'
import hashlib,os,stat,sys
out,base,*roots=sys.argv[1:]
def digest_file(p):
    h=hashlib.sha256()

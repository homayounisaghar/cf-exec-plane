...
port="${VPS_PORT:-22}"
tmp="$(mktemp -d "${RUNNER_TEMP:-/tmp}/cf-stage2-finalize.XXXXXX")"
trap 'rm -rf "$tmp"' EXIT
key="$tmp/key"
kh="$tmp/kh"
printf '%s\n' "$VPS_SSH_KEY" > "$key"
...
ssh -i "$key" -p "$port" -o BatchMode=yes -o IdentitiesOnly=yes -o StrictHostKeyChecking=yes -o UserKnownHostsFile="$kh" -o ConnectTimeout=15 "$VPS_SSH_USER@$VPS_HOST" 'set -euo pipefail
  ...
  [[ -d "$active" && -x "$active/rollback.sh" ]] || { echo "CF_STAGE2_FINALIZE_STATE_MISSING" >&2; exit 50; }
  [[ -s /etc/capability-fabric/trust/deploy-signing.pub && ! -e /etc/capability-fabric/trust/deploy-signing-next.pub ]] || exit 50
  systemctl stop "$watchdog.timer" >/dev/null 2>&1 || true
  rm -rf "$active"
  echo "CF_STAGE2_CUTOVER_COMMITTED=pass"'

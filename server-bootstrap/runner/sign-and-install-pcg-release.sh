...
for n,v in s.items():
    if v.get('network_mode') != 'none':
        raise SystemExit(f'PCG skeleton service must be network_mode none: {n}')
    if v.get('read_only') is not True:
        raise SystemExit(f'PCG skeleton service must be read_only: {n}')
PY

...
for cmd in git ssh-keygen python3 sha256sum docker ssh; do
...
[[ "$pub_type" == ssh-ed25519 && -n "$pub_data" && -z "${pub_extra:-}" ]] || {
  echo "deployment public key must be exactly: ssh-ed25519 <base64>" >&2
  exit 2
}
...
GIT_ASKPASS="$askpass" GIT_TERMINAL_PROMPT=0 git --git-dir="$tmp/repo.git" fetch --quiet --depth=1 "$repo_url" refs/heads/main:refs/heads/source-main
commit="$(git --git-dir="$tmp/repo.git" rev-parse refs/heads/source-main)"
[[ "$commit" =~ ^[0-9a-f]{40}$ ]] || { echo "invalid private-repository head" >&2; exit 3; }

candidate="$tmp/candidate"
mkdir -m 0700 "$candidate"
if ! git --git-dir="$tmp/repo.git" show "$commit:$deploy_dir/manifest.json" > "$candidate/manifest.json" 2>/dev/null; then
  echo "CF_SIGNER_NO_CANDIDATE"
  exit 0
fi

python3 - "$candidate/manifest.json" "$tmp/files" "$tmp/images" <<'PY'
import json,re,sys
manifest,files_path,images_path=sys.argv[1:]
...
while IFS=$'\t' read -r rel expected_hash; do
  ...
  actual_hash="$(sha256sum "$candidate/$rel" | awk '{print $1}')"
  [[ "$actual_hash" == "$expected_hash" ]] || { echo "candidate file hash mismatch" >&2; exit 4; }
done < "$tmp/files"
...
read -r derived_type derived_data derived_extra <<< "$derived"
[[ "$derived_type" == "$pub_type" && "$derived_data" == "$pub_data" && -z "${derived_extra:-}" ]] || {
  echo "deployment signing private/public key mismatch" >&2
  exit 5
}
...
read -r host_key_type host_key_data host_key_extra <<< "$host_key_line"
[[ "$host_key_type" == ssh-ed25519 && -n "$host_key_data" && -z "${host_key_extra:-}" ]] || exit 6
printf '%s %s %s\n' "$VPS_HOST" "$host_key_type" "$host_key_data" > "$known_hosts"
printf '[%s]:%s %s %s\n' "$VPS_HOST" "$port" "$host_key_type" "$host_key_data" >> "$known_hosts"
chmod 0600 "$known_hosts"

existing_sig="$tmp/existing.sig"
set +e
printf '%s\n' "$commit" | ssh -i "$vps_key" -p "$port" -o BatchMode=yes -o IdentitiesOnly=yes \
  -o StrictHostKeyChecking=yes -o UserKnownHostsFile="$known_hosts" -o ConnectTimeout=15 \
  "${VPS_SSH_USER}@${VPS_HOST}" \
  'set -euo pipefail; read -r c; [[ "$c" =~ ^[0-9a-f]{40}$ ]] || exit 2; p="/var/lib/capability-fabric/signatures/${c}.sig"; [[ -s "$p" ]] && cat "$p" || exit 44' \
  > "$existing_sig"
existing_rc=$?
set -e
if [[ "$existing_rc" -eq 0 && -s "$existing_sig" ]]; then
  if ssh-keygen -Y verify -f "$tmp/allowed_signers" -I "$sign_id" -n "$sign_namespace" -s "$existing_sig" < "$candidate/manifest.json" >/dev/null 2>&1; then
...
[[ -s "$candidate/manifest.json.sig" ]] || { echo "signature was not produced" >&2; exit 7; }
ssh-keygen -Y verify -f "$tmp/allowed_signers" -I "$sign_id" -n "$sign_namespace" \
  -s "$candidate/manifest.json.sig" < "$candidate/manifest.json" >/dev/null

{
  printf '%s\n' "$commit"
  cat "$candidate/manifest.json.sig"
} | ssh -i "$vps_key" -p "$port" -o BatchMode=yes -o IdentitiesOnly=yes \
  -o StrictHostKeyChecking=yes -o UserKnownHostsFile="$known_hosts" -o ConnectTimeout=15 \
  "${VPS_SSH_USER}@${VPS_HOST}" \
  'set -euo pipefail; umask 077; read -r c; [[ "$c" =~ ^[0-9a-f]{40}$ ]] || exit 2; d=/var/lib/capability-fabric/signatures; install -d -m 0750 -o root -g root "$d"; t=$(mktemp "$d/.signature.XXXXXX"); trap '\''rm -f "$t"'\'' EXIT; cat > "$t"; grep -q "BEGIN SSH SIGNATURE" "$t"; grep -q "END SSH SIGNATURE" "$t"; chown root:root "$t"; chmod 0640 "$t"; mv -f "$t" "$d/${c}.sig"; trap - EXIT; ls -1t "$d"/*.sig 2>/dev/null | tail -n +33 | xargs -r rm -f'

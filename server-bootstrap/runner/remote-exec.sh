...
case "$key_data" in *[!A-Za-z0-9+/=]*) echo "VPS_HOST_KEY contains malformed key data" >&2; exit 3 ;; esac
...
if [[ -n "${CF_HARDENING_MODE:-}" ]]; then
  remote_env+=("CF_HARDENING_MODE=$CF_HARDENING_MODE")
fi
remote_cmd='env'

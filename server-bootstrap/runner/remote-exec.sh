...
while IFS= read -r line; do
  ...
  key_data="${line#* }"
  case "$key_type" in
    ssh-ed25519|ssh-rsa|rsa-sha2-*|ecdsa-sha2-*) ;;
    *) echo "VPS_HOST_KEY contains an unsupported key type" >&2; exit 3 ;;
  esac
  case "$key_data" in
...
(( valid_host_key_lines > 0 )) || { echo "VPS_HOST_KEY is empty" >&2; exit 3; }

...
release=/var/lib/capability-fabric/releases/onshape-three-session-v46-fabric-shadow
...
name=cf-v46-isolated-startup
[[ -d "$release" && -s "$release/server.js" && -s "$release/fabric-agent.js" ]] || { echo CF_V46_ISOLATED_RELEASE=missing; exit 20; }

tmp="$(mktemp -d /var/lib/capability-fabric/.v46-isolated.XXXXXX)"
...
docker run -d --name "$name" --network bridge --read-only --tmpfs /tmp:rw,nosuid,size=1g --cap-drop ALL --security-opt no-new-privileges:true \
  ...
  -e CF_FABRIC_SIDECAR_URL=http://127.0.0.1:8791 -e CF_FABRIC_AGENT_STATE_DIR=/agent-state -e CF_PUBLIC_SURFACE=shadow \
...
for _ in $(seq 1 90); do
  ...
  [[ "$running" == true ]] || break
  if docker exec "$name" node -e "Promise.all([8788,8789].map(p=>fetch('http://127.0.0.1:'+p+'/').then(async r=>{if(!r.ok||(await r.text())!=='cf-onshape-single ok')throw Error()}))).then(()=>process.exit(0)).catch(()=>process.exit(1))" >/dev/null 2>&1; then ready=yes; break; fi
  sleep 1
done
echo "CF_V46_ISOLATED_READY=$ready"
echo "CF_V46_ISOLATED_STATE=$(docker inspect -f '{{.State.Status}}/{{.State.Running}}/{{.State.ExitCode}}' "$name" 2>/dev/null || echo unavailable)"
echo CF_V46_ISOLATED_LOG_BEGIN
docker logs --tail 160 "$name" 2>&1 | sed -E 's#https?://[^[:space:]"]+#<url>#g'
echo CF_V46_ISOLATED_LOG_END
echo "CF_V46_LIVE_STATE=$(docker inspect -f '{{.State.Status}}/{{.State.Running}}' capability-fabric-onshape-server 2>/dev/null || echo missing)"
[[ "$(docker inspect -f '{{.State.Running}}' capability-fabric-onshape-server 2>/dev/null || echo false)" == true ]] || exit 30
echo CF_V46_LIVE_IMPACT=none

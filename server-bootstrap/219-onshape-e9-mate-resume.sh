...
const {f:fm,s:finalStatus}=fstatus(fin.b,MATE);
if(finalStatus!=="OK"||String((fm.parameters||[]).find(p=>p.parameterId==="translationX")?.expression)!=="20 mm") throw new Error("final mate baseline");

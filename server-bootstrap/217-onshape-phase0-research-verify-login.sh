...
[[ "$(id -u)" -eq 0 ]] || exit 2
[[ "$(git hash-object "$control")" == "$expected_control_blob" ]] || { echo CF_PHASE0_VERIFY_CONTROL=freshness-mismatch; exit 20; }
...
echo CF_PHASE0_VERIFY_PORTS=pass
...
try {
  ...
  console.log("CF_PHASE0_VERIFY_TOOL_CATALOG=pass");
  ...
  console.log("CF_PHASE0_VERIFY_CAPABILITIES="+ids.join(","));
  ...
  console.log("CF_PHASE0_VERIFY_FIXTURE_GUARD=pass");
  ...
  console.log("CF_PHASE0_VERIFY_BUILD_ID="+status.build_id);
  ...
  if(status?.auth?.state!=="PROVEN"){
    ...
    for(let i=0;i<150;i++){
      ...
      if(op.status==="AWAITING_INPUT"){
        ...
        console.log("CF_PHASE0_VERIFY_LOGIN=AWAITING_EMAIL_VERIFICATION");
        awaiting=true;
        terminal=true;
        break;
      }
  ...
  if(!awaiting){
    ...
    console.log("CF_PHASE0_VERIFY_AUTH=PROVEN");
  }
} finally {
...
if (( node_rc == 42 )); then
  echo CF_PHASE0_VERIFY_LOGIN=awaiting-email-verification
  exit 42
fi
...
echo CF_PHASE0_VERIFY_PRODUCTION_UNCHANGED=pass
echo CF_PHASE0_VERIFY_LOGIN=pass

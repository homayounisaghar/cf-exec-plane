...
EXPECTED_CONTROL=ca5c6d0562ea29e9ab26897a6b7fb137bfd1649d
EXPECTED_MANIFEST=e64a1053c02a5abf6becd09275b91768f8642a724c7f0db6d4106a8cbe19583f

active="$(readlink -f "$ACTIVE")"
[[ "$(basename "$active")" == "onshape-vps-hardened-production-r6" ]] || exit 20
...
assert x["controlRevision"]==546 and a["productionEpoch"]==17
...
const client=new Client({name:"cf-r6-cohort-reauth",version:"1.0.0"});
...
async function waitOp(id,sessionId,timeoutMs=120000){
  ...
  while(Date.now()<deadline){
    ...
    if(status==="AWAITING_INPUT"){
      console.log("CF_R6_REAUTH_AWAITING_INPUT_SESSION="+sessionId);
      console.log("CF_R6_REAUTH_AWAITING_INPUT_KIND="+String(op.input_required||v.input_required||""));
...
let p=await pool();
if(p.build_id!=="onshape-vps-hardened-r6") throw new Error("wrong build");
...
for(const [index,sessionId] of ["session-1","session-2","session-3"].entries()){
  ...
  console.log("CF_R6_REAUTH_OPERATION_"+sessionId.replace("-","_")+"="+opId);
  ...
  console.log("CF_R6_REAUTH_TERMINAL_"+sessionId.replace("-","_")+"="+done.terminal);
  console.log("CF_R6_REAUTH_AUTH_"+sessionId.replace("-","_")+"="+String(s?.auth?.state));
  ...
  if(index<2){
    ...
    console.log("CF_R6_REAUTH_FINAL_POOL=3-of-3-PROVEN-idle");
    console.log("CF_R6_REAUTH_FINGERPRINTS_DISTINCT=true");
  }
}
await client.close();
console.log("CF_R6_REAUTH=pass");
NODE
node_rc=$?
set -e
...
mv -f "$tmp" "$GATE"
restore=no
echo CF_R6_REAUTH_WINDOW=closed

if [[ "$node_rc" -eq 42 ]]; then
  echo CF_R6_REAUTH=awaiting-user-verification
  exit 42
fi
[[ "$node_rc" -eq 0 ]] || exit "$node_rc"
echo CF_R6_REAUTH_GATE_RESTORED=pass

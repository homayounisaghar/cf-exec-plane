...
[[ "$(basename "$active")" == "onshape-vps-hardened-production-r7" ]] || exit 20
...
assert x["controlRevision"]==550 and a["productionEpoch"]==21
assert a["mode"]=="VPS_PRODUCTION" and a["materialAuthority"]=="vps-fabric"
assert a["planes"]["vps-fabric"]["releaseSequence"]==72
assert a["planes"]["vps-fabric"]["releaseId"]=="onshape-vps-hardened-production-r7"
...
const c=new Client({name:"cf-r7-cohort-reauth",version:"1.0.0"});
...
const waitOp=async(id,sid,timeoutMs=120000)=>{
 ...
 while(Date.now()<deadline){
  ...
  if(status==="AWAITING_INPUT"){
   console.log("CF_R7_REAUTH_AWAITING_INPUT_SESSION="+sid);
...
let p=await pool();
if(p.build_id!=="onshape-vps-hardened-r7") throw new Error("wrong build "+String(p.build_id));
...
for(const [index,sid] of ["session-1","session-2","session-3"].entries()){
 ...
 console.log("CF_R7_REAUTH_OPERATION_"+sid.replace("-","_")+"="+opId);
 const done=await waitOp(opId,sid);
 if(done.terminal==="AWAITING_INPUT"){await c.close();process.exit(42);}
 p=await pool();
 const s=state(p,sid);
 console.log("CF_R7_REAUTH_TERMINAL_"+sid.replace("-","_")+"="+done.terminal);
 console.log("CF_R7_REAUTH_AUTH_"+sid.replace("-","_")+"="+String(s?.auth?.state));
 ...
 if(index<2){
   ...
   console.log("CF_R7_REAUTH_FINAL_POOL=3-of-3-PROVEN-idle");
   console.log("CF_R7_REAUTH_FINGERPRINTS_DISTINCT=true");
 }
}
await c.close();
console.log("CF_R7_REAUTH=pass");
NODE
node_rc=$?
set -e
...
tmp="$GATE.tmp.r7reauth.$$"; printf '%s\n' RELEASE_IN_PROGRESS >"$tmp"; chmod 0600 "$tmp"; chown root:root "$tmp"; mv -f "$tmp" "$GATE"
restore=no
echo CF_R7_REAUTH_WINDOW=closed
if [[ "$node_rc" -eq 42 ]]; then echo CF_R7_REAUTH=awaiting-user-verification; exit 42; fi
[[ "$node_rc" -eq 0 ]] || exit "$node_rc"
echo CF_R7_REAUTH_GATE_RESTORED=pass

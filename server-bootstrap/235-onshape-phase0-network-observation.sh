...
try{
  ...
  const meta=await native("page.evaluate",{expression:String.raw`(() => {const c=document.querySelector("#canvas"),r=c?.getBoundingClientRect();return r?{x:r.x,y:r.y,w:r.width,h:r.height,view:c.getAttribute("data-view-shown")}:null})()`});
  const cv=meta.value;

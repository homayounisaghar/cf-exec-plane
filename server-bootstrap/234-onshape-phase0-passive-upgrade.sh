...
old_candidate="d882fa763b3a55be5a18d6e196d028ff4f77ea07"
...
print("CF_PHASE0_PASSIVE_UPGRADE_AUTHORITY=pass")
PY
...
tmp="$(mktemp -d "$root/.passive-upgrade.XXXXXX")"; trap 'rm -rf "$tmp"' EXIT
ask="$tmp/askpass"
cat >"$ask" <<'ASK'
#!/usr/bin/env bash
case "${1:-}" in
...
grep -Fq '"network.snapshot"' "$release/server-deploy/current/server.js"
...
echo CF_PHASE0_PASSIVE_UPGRADE=pass

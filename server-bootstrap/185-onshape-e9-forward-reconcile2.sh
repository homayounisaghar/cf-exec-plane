...
if systemctl is-active --quiet "$SERVICE"; then echo CF_E9_RECON2_PULL_SERVICE=busy >&2; exit 20; fi
echo CF_E9_RECON2_TIMER=stopped
...
ORDER BY i.rowid DESC LIMIT 30
""", ("%"+did+"%",)).fetchall()
print("CF_E9_RECON2_DB_ROWS="+str(len(rows)))
for r in rows:
 d=dict(r)
 ...
 print("CF_E9_RECON2_DB_ROW="+json.dumps(d,separators=(',',':'),sort_keys=True))
c.close()
PY
...
for p in root.rglob("*.json"):
 ...
 if isinstance(v,dict) and v.get("budgetId")=="e9-selector-durability-slice-a-20260925":
  items.append((str(p.relative_to(root)),v))
print("CF_E9_RECON2_BUDGET_ROWS="+str(len(items)))
for name,v in items:
 print("CF_E9_RECON2_BUDGET_ROW="+json.dumps({"file":name,**v},separators=(',',':'),sort_keys=True))
PY
...
const client=new Client({name:"cf-e9-forward-reconcile2",version:"1.0.0"});
...
if(fr?.outcome?.state!=="ACHIEVED"||fe.httpStatus!==200||fe.effectSent!==false) throw new Error("feature read");
if(fe.apiVersion!=="v17"||fe.observedApiVersion!=="v17"||fe.apiVersionMatched!==true) throw new Error("feature version");
...
console.log("CF_E9_RECON2_DEPTH="+depth);
console.log("CF_E9_RECON2_FEATURE_STATUS="+String(feat.featureStatus||""));
console.log("CF_E9_RECON2_MICROVERSION="+String(fb.sourceMicroversion||""));
...
if(dr?.outcome?.state!=="ACHIEVED"||de.httpStatus!==200||de.effectSent!==false) throw new Error("body read");
...
if(!best||best.d>1e-5) throw new Error("edge missing");
console.log("CF_E9_RECON2_TOP_Z="+String(best.z));
console.log("CF_E9_RECON2_READBACK=pass");
await client.close();
NODE
...
restore=no
trap - EXIT
echo CF_E9_RECON2_GATE=active
echo CF_E9_RECON2_TIMER=stopped
echo CF_E9_RECON2=pass

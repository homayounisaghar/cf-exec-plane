...
[[ "$(id -u)" -eq 0 ]] || { echo CF_STAGE_R2_REQUIRES_ROOT >&2; exit 2; }

source_commit=ef5634b94fa7e04af1edfebfefa3f7957fce5035
manifest_path=server-deploy/candidates/onshape-vps-hardened-production-r2/manifest.json
expected_manifest_sha=71bc73a66b7aa05fb726b0e4cb38b872f6c10eeea1a1152a4367fab2adcdca0c
release_id=onshape-vps-hardened-production-r2
...
[[ ! -e "$release_gate" ]] || { echo CF_STAGE_R2_RELEASE_GATE=active >&2; exit 20; }
...
print("CF_STAGE_R2_ACTIVE_BEFORE=seq63")
PY
...
echo "CF_STAGE_R2_SOURCE_COMMIT=$fetched"
...
[[ "$manifest_sha" == "$expected_manifest_sha" ]] || { echo CF_STAGE_R2_MANIFEST_SHA=mismatch >&2; exit 22; }
echo "CF_STAGE_R2_MANIFEST_SHA256=$manifest_sha"

sig=/var/lib/capability-fabric/signatures/${manifest_sha}.sig
[[ -s "$sig" ]] || { echo CF_STAGE_R2_SIGNATURE=missing >&2; exit 23; }
pub="$(tr -d '\r\n' < "$trust")"
...
echo CF_STAGE_R2_SIGNATURE=pass
...
print("CF_STAGE_R2_MANIFEST_IDENTITY=pass")
PY

if [[ -e "$target" ]]; then
  [[ -d "$target" ]] || exit 24
  stage="$target"
  echo CF_STAGE_R2_WRITE=UNCHANGED
else
  ...
  while IFS=$'\t' read -r rel expected; do
    ...
    [[ "$actual" == "$expected" ]] || { echo "CF_STAGE_R2_FILE_HASH_MISMATCH=$rel" >&2; rm -rf "$stage"; exit 25; }
...
print("CF_STAGE_R2_COMPOSE_RESOLUTION=pass")
print("CF_STAGE_R2_IMAGE_SET=manifest-exact")
PY

if [[ "$stage" != "$target" ]]; then
  mv "$stage" "$target"
  echo CF_STAGE_R2_WRITE=CHANGED
fi

active_after="$(readlink -f "$active")"
previous_after="$(readlink -f "$previous" 2>/dev/null || true)"
[[ "$active_after" == "$active_before" ]] || { echo CF_STAGE_R2_ACTIVE_POINTER=changed >&2; exit 27; }
[[ "$previous_after" == "$previous_before" ]] || { echo CF_STAGE_R2_PREVIOUS_POINTER=changed >&2; exit 27; }
[[ ! -e "$release_gate" ]] || { echo CF_STAGE_R2_RELEASE_GATE=changed >&2; exit 27; }
[[ "$(sha256sum "$control" | awk '{print $1}')" == "$control_sha_before" ]] || { echo CF_STAGE_R2_RUNTIME_CONTROL=changed >&2; exit 27; }
for c in "${containers[@]}"; do
  docker inspect -f '{{.Name}}|{{.Id}}|{{.State.StartedAt}}|{{.Image}}' "$c" >>"$after_file"
done
cmp -s "$before_file" "$after_file" || { echo CF_STAGE_R2_CONTAINERS=changed >&2; exit 27; }

echo CF_STAGE_R2_ACTIVE_POINTER=unchanged
echo CF_STAGE_R2_PREVIOUS_POINTER=unchanged
echo CF_STAGE_R2_RELEASE_GATE=clear
echo CF_STAGE_R2_RUNTIME_CONTROL=unchanged
echo CF_STAGE_R2_CONTAINERS=unchanged
echo CF_STAGE_R2_AUTHORITY_AFTER=android-epoch1
echo CF_STAGE_R2=pass

...
docker run -d --name "$name" --network bridge --read-only --tmpfs /tmp:rw,nosuid,size=1g --cap-drop ALL --security-opt no-new-privileges:true \
  ...
  "$image" sh -lc 'umask 077 && chmod 0700 /profile /run/onshape-secrets && mkdir -p /tmp/app && cp /release/package.json /release/server.js /release/gateway.js /release/core.js /release/browser.js /release/session-pool.js /release/onshape-request.cjs /release/fabric-agent.js /tmp/app/ && cd /tmp/app && PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 npm install --omit=dev --ignore-scripts --no-audit --no-fund --package-lock=false && node server.js >/tmp/server.log 2>&1 & server_pid=$!; trap "kill $server_pid >/dev/null 2>&1 || true" EXIT TERM INT; for _ in $(seq 1 120); do curl -fsS http://127.0.0.1:8788/ >/dev/null 2>&1 && break; kill -0 $server_pid 2>/dev/null || { cat /tmp/server.log >&2; exit 1; }; sleep 1; done; exec node gateway.js' >/dev/null
